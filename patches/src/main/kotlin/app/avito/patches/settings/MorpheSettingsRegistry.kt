...
object MorpheSettingsRegistry {
    ...
    fun addSwitch(key: String, title: String, summary: String? = null, default: Boolean = true) {
        entries[key] = Switch(key, title, summary, default)
    }
    ...
    fun toJson(): String = entries.values.joinToString(",", "[", "]") { entry ->
        when (entry) {
            is Switch -> buildString {
                ...
                append(",\"default\":").append(entry.default).append("}")
            }
